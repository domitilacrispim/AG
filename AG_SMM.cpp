...
void avalia_ind_alg ( individuo *x ){ // agrega nota ao individuo
        int fit=5;
        if ((x->dna[3]+x->dna[1])==x->dna[4]) fit--;
        if (((x->dna[2]+x->dna[7])%10)==x->dna[1] || ((x->dna[2]+x->dna[7]+1)%10)==x->dna[1] ) fit--;
        if (((x->dna[1]+x->dna[6])%10)==x->dna[2] || ((x->dna[1]+x->dna[6]+1)%10)==x->dna[2] ) fit--;
        if (((x->dna[0]+x->dna[5])%10)==x->dna[6] || ((x->dna[0]+x->dna[5]+1)%10)==x->dna[6] ) fit--;
        if (0==x->dna[5] || 1==x->dna[5] ) fit--;
...
void cruzamento ( individuo r[tam_pop], double tx, individuo filhos[tam_pop]){ // pmx
	...
	for (int y=0; y<news; y++){
		...
		int ind=0;
		int vis[10], p=rand()%6, aux1, aux2, pro[10];// printf("%d\n", p);
		...
		for ( int i=0; i<3; i++){
			...
			for ( int op=0; op<10; op++){
				if( f1.dna[op]==aux2 && v1 ){
				//	printf("%d %d %d aux1\n", f1.dna[op], f1.dna[p+i], aux1);

					f1.dna[p+i]=f1.dna[op];
					f1.dna[op]=aux1;
				//	printf("%d %d %d aux1\n", f1.dna[op], f1.dna[p+i], aux1);

					v1=!v1;
				}
				if( aux1==f2.dna[op] && v2){
					f2.dna[p+i]=f2.dna[op];
					f2.dna[op]=aux2;
					v2=!v2;
				}
			}
		...
		//for ( int o =0; o<10;o++){	printf("f1 %d f2 %d\n", f1.dna[o],f2.dna[o]);	}
